...
- лента «Последние работы» с сортировкой по времени публикации;
- увеличение изображения по щелчку и просмотр соседних работ выбранной рубрики;
...
### Первые экраны Design Lab

Снимки сделаны в светлой теме после входа в демонстрационный аккаунт. На каждом
экране показана одна и та же лента, поэтому оформление можно сравнивать без
изменения содержимого.

#### Классический

Основной рабочий интерфейс: компактный заголовок, фильтры рубрик и равномерная
сетка карточек. Этот режим служит безопасным резервным вариантом.

![Первый экран классического интерфейса S-Art](docs/screenshots/design-classic-light.png)

#### Digital Gallery

Выставочная подача с увеличенными полями, спокойной типографикой и акцентом на
самих работах. Маршрут: `/ui/gallery`.

![Первый экран Digital Gallery](docs/screenshots/design-gallery-light.png)

#### Neo-Art Studio

Контрастная студийная композиция с крупным заголовком и более плотными
карточками. Светлая и тёмная темы переключаются общей кнопкой приложения.

![Первый экран Neo-Art Studio](docs/screenshots/design-neo-light.png)

#### Creative Canvas

Альбомная компоновка с рукописным заголовком, мягкими цветными рубриками и
карточками, напоминающими листы творческого портфолио.

![Первый экран Creative Canvas](docs/screenshots/design-canvas-light.png)

### Просмотр работ

- работа открывается в полноэкранном окне по щелчку на изображение;
- повторный щелчок по изображению, кнопка закрытия или `Esc` возвращают в ленту;
- кнопки `<<` и `>>`, а также клавиши `←` и `→`, циклически переключают работы;
- при активном фильтре переключение остаётся внутри выбранной рубрики;
- подпись показывает название, рубрику и позицию работы в текущей подборке;
- карточка отображает относительное время публикации, а точная дата доступна
...
Демонстрационное наполнение создаётся идемпотентно: повторный запуск seed не
дублирует записи. В каждой рубрике есть тестовые публикации разных авторов. Для
«Городских сюжетов» дополнительно подготовлены шесть работ о Берлине, Лондоне,
Риме и Стамбуле.
...
Для публичной демонстрации удобнее разделить компоненты:

| Компонент | Возможные сервисы | Примечание |
|---|---|---|
| Vue frontend | Cloudflare Pages, Vercel | Статическая production-сборка |
| FastAPI | Render, Koyeb | Бесплатные инстансы могут «засыпать» |
| PostgreSQL | Neon, Supabase | Бесплатные тарифы имеют лимиты хранения и вычислений |
| Redis | Upstash | Подходит для небольшого демонстрационного кеша |
| Изображения | Cloudinary либо S3-совместимое хранилище | Нельзя полагаться на локальный диск контейнера |

Альтернатива — один Always Free VM в Oracle Cloud с текущим Docker Compose, но
он требует самостоятельного администрирования, HTTPS, backup и мониторинга.
Перед любым интернет-размещением необходимо выполнить требования раздела
«Безопасность» и отключить либо изолировать публичные демонстрационные пароли.
