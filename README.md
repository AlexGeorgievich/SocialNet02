...
В Windows после первой сборки приложение запускается двойным щелчком по
`start.bat`. Сценарий использует готовые локальные Docker-образы (`--no-build`),
поэтому повторный запуск не зависит от доступности Docker Hub. Он не запускает
отдельные Vite/Uvicorn-процессы, поэтому frontend всегда остаётся на
`http://localhost:5173/`.

После изменения исходного кода или при первом запуске нужна сборка с доступом в
Интернет:
...
Классический интерфейс остаётся основным и резервным:

- `/` — классический S-Art;
- `/ui/gallery` — минималистичная Digital Gallery;
- `/ui/neo` — тёмная неоновая Neo-Art Studio;
- `/ui/canvas` — художественный альбом Creative Canvas.

Каждый художественный вариант является отдельным lazy-loaded Vue-маршрутом со
своим scoped CSS. При переключении предыдущая страница размонтируется, поэтому
сетки и стили разных вариантов не перестраиваются и не смешиваются. Выбор
сохраняется в `localStorage` и доступен также в настройках профиля.

При ошибке загрузки маршрута или Vue-компонента приложение автоматически
возвращается на классический `/`. Прямой аварийный адрес:
`http://localhost:5173/?design_fallback=1`.
